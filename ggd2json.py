...
def parseRecord(record: dict):
    ...
    for k in ["title", "impressum", "collate", "description", "comments", "pages"]:
    ...
    record["created"] = datetime.strptime(record["created"], "%d-%m-%Y").strftime(
        "%Y-%m-%d"
    )
    record["modified"] = datetime.strptime(record["modified"], "%d-%m-%Y").strftime(
        "%Y-%m-%d"
    )
    ...
    record["melody"] = [
        {"label": i, "liederenbank": ID2MELODIE.get(record["id"], {}).get(i)}
        for i in record.get("melody", [])
    ]

    # persons/roles
    if record.get("person"):
        record["person"] = getPersons(
            record["person"], recordID=record["id"], getRole=True
        )
    ...
    for k in ["item_cbg", "item_saa", "item_kb", "item_mmw", "item_mnl"]:
        if record.get(k):
            ...
            for i in items:
                ...
                record["item"].append(
                    {"location": i, "holdingArchive": k2archive[k], "comment": comment}
                )
